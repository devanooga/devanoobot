 ...
 * @param {string} data.gitmsg - The message to be used in the commit.
...
module.exports = data => {
  ...
  // Commit changes.
  if (shell.exec(`git commit -am '${data.gitmsg}'`).code !== 0) {
  ...
  var pull_request = shell.exec(`hub pull-request -m '${data.gitmsg}'`)
